...
<div class="modal-box" id="orderModal">
  ...
  <div class="modal-form" id="orderFormPanel">
    ...
    <div id="buyerFieldsTokoObat" style="display:none;">
      ...
      <input id="f_nama_toko_obat" type="text" class="form-inp" placeholder="Nama toko obat">
      <label class="form-lbl">Nama Penanggung Jawab <span style="color:#ef4444;">*</span></label>
      <input id="f_pj_toko_obat" type="text" class="form-inp" placeholder="Nama penanggung jawab">
      <label class="form-lbl">No. HP / WA <span style="color:#ef4444;">*</span></label>
      <input id="f_hp_toko_obat" type="tel" class="form-inp" placeholder="08xxxxxxxxxx">
      <label class="form-lbl">Alamat Toko Obat <span style="color:#ef4444;">*</span></label>
      <textarea id="f_alamat_toko_obat" rows="3" class="form-inp" style="resize:vertical;" placeholder="Alamat lengkap toko obat"></textarea>
      <div class="form-row">
        <div><label class="form-lbl">Kecamatan</label><input id="f_kec_toko_obat" type="text" class="form-inp" placeholder="Kecamatan"></div>
        <div><label class="form-lbl">Kota / Kab</label><input id="f_kota_toko_obat" type="text" class="form-inp" placeholder="Kota / Kabupaten"></div>
      </div>
      <label class="form-lbl">NIB <span style="color:#ef4444;">*</span></label>
      <input id="f_nib_toko_obat" type="text" class="form-inp" placeholder="Nomor NIB">
      <label class="form-lbl">NPWP Sarana <span style="color:#ef4444;">*</span></label>
      <input id="f_npwp_sarana_toko_obat" type="text" class="form-inp" placeholder="Nomor NPWP sarana">
      <label class="form-lbl">KTP Pemilik <span style="color:#ef4444;">*</span></label>
      <input id="f_ktp_pemilik_toko_obat" type="text" class="form-inp" placeholder="Nomor KTP pemilik">
      <label class="form-lbl">SIPTTK PJ + KTP PJ <span style="color:#ef4444;">*</span></label>
      <input id="f_sipttk_pj_toko_obat" type="text" class="form-inp" placeholder="Nomor SIPTTK PJ + KTP PJ">
    </div>
...
function toggleBuyerType() {
  ...
  document.getElementById('buyerFieldsTokoObat').style.display = type === 'toko_obat' ? '' : 'none';
...
function buildReceiptPdf(orderData) {
  ...
  writeMeta('Jenis Pembeli', getBuyerTypeLabel(orderData.buyer_type));
  if (orderData.buyer_name) {
    const buyerNameLabel = orderData.buyer_type === 'pbf'
      ? 'Nama Pemesan'
      : (orderData.buyer_type === 'toko_obat' ? 'Penanggung Jawab' : 'Pembeli');
    writeMeta(buyerNameLabel, orderData.buyer_name);
  }
  if (orderData.buyer_type === 'toko_obat' && orderData.store_name) writeMeta('Nama Toko Obat', orderData.store_name);
  if (orderData.buyer_type === 'pbf') {
  ...
  } else if (orderData.buyer_type === 'toko_obat') {
    if (orderData.sia) writeMeta('NIB', orderData.sia);
    if (orderData.no_izin_pbf) writeMeta('NPWP Sarana', orderData.no_izin_pbf);
    if (orderData.apj) writeMeta('KTP Pemilik', orderData.apj);
    if (orderData.sipa) writeMeta('SIPTTK PJ + KTP PJ', orderData.sipa);
  }
  drawLine();
  startProductSection();
...
async function submitOrder() {
  ...
  if (jenis === 'apotik') {
    ...
    payload.buyer_name = document.getElementById('f_pj_toko_obat').value.trim() || payload.store_name;
    payload.phone    = document.getElementById('f_hp_toko_obat').value.trim();
    payload.address  = document.getElementById('f_alamat_toko_obat').value.trim();
    payload.kecamatan = document.getElementById('f_kec_toko_obat').value.trim();
    payload.kota     = document.getElementById('f_kota_toko_obat').value.trim();
    payload.sia      = document.getElementById('f_nib_toko_obat').value.trim();
    payload.no_izin_pbf = document.getElementById('f_npwp_sarana_toko_obat').value.trim();
    payload.apj      = document.getElementById('f_ktp_pemilik_toko_obat').value.trim();
    payload.sipa     = document.getElementById('f_sipttk_pj_toko_obat').value.trim();
    if (!payload.store_name || !payload.buyer_name || !payload.phone || !payload.address || !payload.sia || !payload.no_izin_pbf || !payload.apj || !payload.sipa) {
      err.textContent = 'Semua field Toko Obat wajib diisi.'; err.style.display = 'block'; return;
    }
  } else if (jenis === 'pbf') {
...
function openWhatsAppOrder() {
  ...
  msg += `---\n*Total: ${rp(total)}*\n\n*Data ${window.orderPayload?.buyer_type === 'apotik' ? 'Apotik' : window.orderPayload?.buyer_type === 'toko_obat' ? 'Toko Obat' : 'Pemesan'}:*\n`;
  ...
  } else if (window.orderPayload?.buyer_type === 'toko_obat') {
    msg += `- Toko Obat: ${window.orderPayload?.store_name || '-'}\n- Penanggung Jawab: ${window.orderPayload?.buyer_name || '-'}\n- HP/WA: ${window.orderPayload?.phone || '-'}\n- Alamat: ${window.orderPayload?.address || '-'}\n- NIB: ${window.orderPayload?.sia || '-'}\n- NPWP Sarana: ${window.orderPayload?.no_izin_pbf || '-'}\n- KTP Pemilik: ${window.orderPayload?.apj || '-'}\n- SIPTTK PJ + KTP PJ: ${window.orderPayload?.sipa || '-'}\n`;
...
<?php /**PATH C:\Users\Ali Attaziri\medistrafarma\resources\views\partials\cart.blade.php ENDPATH**/ ?>