...
.prod-section::before {
    ...
    position: absolute;
    inset: 0;
    background: radial-gradient(circle at 15% 15%, rgba(45,212,191,0.28), transparent 18%),
                radial-gradient(circle at 85% 20%, rgba(251,146,60,0.20), transparent 22%),
                radial-gradient(circle at 50% 100%, rgba(59,130,246,0.10), transparent 24%);
    pointer-events: none;
}
.prod-grid { position: relative; z-index: 1; display: grid; grid-template-columns: repeat(auto-fill, minmax(185px, 1fr)); gap: 1rem; margin-bottom: 1.5rem; }
.prod-card {
...
.why-section::before {
    ...
    position: absolute;
    inset: 0;
    background: radial-gradient(circle at 15% 20%, rgba(45,212,191,0.24), transparent 22%),
                radial-gradient(circle at 80% 15%, rgba(249,115,22,0.18), transparent 20%),
                radial-gradient(circle at 70% 80%, rgba(16,185,129,0.14), transparent 28%);
    pointer-events: none;
}
.why-grid { position: relative; z-index: 1; display: flex; justify-content: center; gap: 2.5rem; flex-wrap: wrap; }
...
.cta-section::before {
    ...
    position: absolute;
    inset: 0;
    background: radial-gradient(circle at 20% 30%, rgba(45,212,191,0.28), transparent 22%),
                radial-gradient(circle at 75% 25%, rgba(251,146,60,0.18), transparent 24%),
                radial-gradient(circle at 50% 85%, rgba(16,185,129,0.14), transparent 30%);
    pointer-events: none;
}
.cta-box {
    ...
    box-shadow: 0 10px 26px rgba(15, 118, 110, 0.10);
    opacity: 1;
    position: relative;
    overflow: hidden;
}
...
.cat-section::before {
    ...
    position: absolute;
    inset: 0;
    background: radial-gradient(circle at 12% 10%, rgba(45,212,191,0.12), transparent 22%),
                radial-gradient(circle at 85% 15%, rgba(251,146,60,0.08), transparent 24%);
    pointer-events: none;
}
.cat-grid { position: relative; z-index: 1; display: grid; grid-template-columns: repeat(6, 1fr); gap: 0.75rem; }
.cat-card {
    display: flex; flex-direction: column; align-items: center; gap: 0.5rem;
    padding: 1rem 0.5rem; background: linear-gradient(180deg, #fffdfb 0%, #f3fffb 100%); border-radius: 14px;
    border: 1.5px solid rgba(15, 118, 110, 0.12); text-decoration: none; color: #374151;
    transition: all 0.25s; text-align: center;
    box-shadow: 0 6px 18px rgba(15, 118, 110, 0.08);
}
.cat-card:hover { background: linear-gradient(180deg, #fff7ef 0%, #f0fdf5 100%); border-color: rgba(20,184,166,.35); color: #0f766e; transform: translateY(-3px); box-shadow: 0 8px 20px rgba(20,184,166,0.10); }
                ...
                <?php $__currentLoopData = $latestNews; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $news): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e(route('news.index', ['news_id' => $news->id])); ?>" class="news-card" style="text-decoration: none; color: inherit; display: block; min-width: 0; border-radius: 16px; overflow: hidden; transition: transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1), border-color 0.3s ease, box-shadow 0.3s ease; scroll-snap-align: start; position: relative; z-index: 2;">
                        ...
                        <div style="padding: 1.1rem 1rem 1.25rem; background: #ffffff; position: relative; z-index: 2; border-top: 1px solid rgba(15, 118, 110, 0.08);">
                            <div class="news-meta">
                                <div class="news-date"><?php echo e(optional($news->tanggal)->translatedFormat('d M Y') ?? $news->created_at->translatedFormat('d M Y')); ?></div>
                            </div>
