...
<style>
    ...
    .about-section-label { font-size: 0.72rem; font-weight: 800; letter-spacing: 0.1em; text-transform: uppercase; color: #d9f7ff; margin-bottom: 0.4rem; display: block; }
    .about-section-title { font-size: clamp(1.35rem, 2.5vw, 1.8rem); font-weight: 800; color: #ffffff; margin-bottom: 0.5rem; line-height: 1.25; }
    .about-section-sub { font-size: 0.95rem; color: #f0f9ff; line-height: 1.75; max-width: 640px; }

    .abt-info-grid { display: grid; grid-template-columns: 1.15fr 0.85fr; gap: 1.25rem; margin-bottom: 1.5rem; }
    .abt-card {
        background: linear-gradient(135deg, rgba(9, 35, 58, 0.72), rgba(12, 95, 152, 0.62));
        border: 1px solid rgba(186, 230, 253, 0.22);
        border-radius: 20px; padding: 1.75rem; box-shadow: 0 14px 32px rgba(7, 31, 54, 0.14);
        backdrop-filter: blur(2px);
    }
    .abt-card h2 { font-size: 1.1rem; font-weight: 800; color: #f8fdff; margin-bottom: 0.75rem; display: flex; align-items: center; gap: 0.5rem; }
    .abt-card h2 i { color: #bae6fd; font-size: 1rem; }
    .abt-card p, .abt-card li { color: #e2f3ff; line-height: 1.8; font-size: 0.93rem; }
    ...
    .abt-vm-card.misi {
        background: linear-gradient(135deg, rgba(11, 77, 117, 0.74), rgba(17, 94, 89, 0.68));
        border: 1px solid rgba(186, 230, 253, 0.2);
        box-shadow: 0 14px 32px rgba(7, 31, 54, 0.12);
    }
    ...
    .abt-vm-card.misi h3 { color: #f8fdff; }
    .abt-vm-card.misi h3 i { color: #bae6fd; }
    ...
    .abt-misi-list li { display: flex; align-items: flex-start; gap: 0.6rem; font-size: 0.9rem; color: #e2f3ff; margin-bottom: 0.6rem; line-height: 1.65; }
    .abt-misi-list li i { color: #bae6fd; margin-top: 0.15rem; flex-shrink: 0; }

    .abt-nilai-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 1.5rem; }
    .abt-nilai-card {
        background: linear-gradient(135deg, rgba(11, 77, 117, 0.72), rgba(14, 116, 110, 0.54));
        border: 1px solid rgba(186, 230, 253, 0.18);
        border-radius: 18px; padding: 1.4rem 1rem; text-align: center; box-shadow: 0 10px 24px rgba(7, 31, 54, 0.1);
    }
    .abt-nilai-icon { width: 52px; height: 52px; border-radius: 14px; background: linear-gradient(135deg, rgba(224, 242, 254, 0.2), rgba(125, 211, 252, 0.18)); display: flex; align-items: center; justify-content: center; margin: 0 auto 0.85rem; font-size: 1.3rem; color: #e0f2fe; }
    .abt-nilai-card h4 { font-size: 0.88rem; font-weight: 800; color: #f8fdff; margin-bottom: 0.35rem; }
    .abt-nilai-card p { font-size: 0.78rem; color: #e2f3ff; line-height: 1.6; margin: 0; }

    .abt-struktur-wrap {
        background: linear-gradient(135deg, rgba(3, 19, 36, 0.95) 0%, rgba(8, 44, 70, 0.94) 36%, rgba(11, 92, 137, 0.92) 100%);
        border: 1px solid rgba(186, 230, 253, 0.2);
        border-radius: 24px; padding: 2rem; box-shadow: 0 20px 40px rgba(3, 19, 36, 0.2); margin-bottom: 1.5rem;
    }
    .abt-struktur-img { border-radius: 16px; overflow: hidden; border: 2px solid rgba(186, 230, 253, 0.28); margin-top: 1.25rem; box-shadow: 0 8px 28px rgba(37, 99, 235, 0.08); }
    .abt-struktur-img img { width: 100%; display: block; object-fit: contain; max-height: 520px; background: rgba(255,255,255,0.9); }
    .abt-struktur-note { display: flex; align-items: center; gap: 0.5rem; margin-top: 0.85rem; font-size: 0.82rem; color: #ecfeff; font-weight: 600; }
    .abt-struktur-note i { color: #7dd3fc; }

    .abt-gallery-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-bottom: 1.5rem; }
    .abt-gallery-item {
        border-radius: 18px; overflow: hidden; border: 1px solid rgba(186, 230, 253, 0.18); background: rgba(8, 27, 46, 0.48); box-shadow: 0 10px 26px rgba(7, 31, 54, 0.08);
    }
    .abt-gallery-item img { width: 100%; height: 200px; object-fit: cover; display: block; }
    .abt-gallery-caption { padding: 0.75rem 1rem; font-size: 0.82rem; font-weight: 600; color: #e0f2fe; background: rgba(8, 27, 46, 0.56); }

    .abt-branch-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem; }
    .abt-branch-card {
        background: linear-gradient(135deg, rgba(9, 35, 58, 0.72), rgba(12, 95, 152, 0.58));
        border: 1px solid rgba(186, 230, 253, 0.18);
        border-radius: 20px; padding: 1.5rem; box-shadow: 0 12px 28px rgba(7, 31, 54, 0.1);
    }
    .abt-branch-head { display: flex; align-items: center; gap: 0.65rem; margin-bottom: 0.85rem; }
    .abt-branch-icon { width: 40px; height: 40px; border-radius: 11px; background: linear-gradient(135deg, rgba(224, 242, 254, 0.14), rgba(125, 211, 252, 0.08)); display: flex; align-items: center; justify-content: center; font-size: 1rem; color: #e0f2fe; flex-shrink: 0; }
    .abt-branch-card h3 { font-size: 0.95rem; font-weight: 800; color: #f8fdff; margin: 0; }
    .abt-branch-card .abt-branch-sub { font-size: 0.82rem; color: #dbeafe; margin: 0.15rem 0 0; }
    .abt-branch-card p { font-size: 0.85rem; color: #e2f3ff; margin: 0; line-height: 1.6; }
